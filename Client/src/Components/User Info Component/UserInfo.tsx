...
import { steps , ht,wt,age, goal } from "./userInfoProps";
    ...
    <div className="flex flex-col bg-zinc-950 overflow-hidden h-screen">
        ...
        <div className="h-screen flex flex-col mt-20 items-center text-center gap-3 ">
            ...
            {step<3 && <div className="flex flex-row gap-2  bg-lime-500 p-2 rounded-md mt-7">
                <button onClick={handlebtn} className={`cursor-pointer px-10 rounded-md py-2 ${active?"text-white bg-black":"text-black bg-none"} duration-200`}>{step===1?"ft":"kg"}</button>
                <button className="text-black px-4">|</button>
                <button onClick={handlebtn} className={`cursor-pointer px-10 py-2 duration-200 ease-in ${active?"text-black bg-none":"text-white bg-black rounded-md"}`}>{step===1?"cm":"lbs"}</button>
            </div> }
